...
public class ProductOperationService {
     ...
     * 基于语义相似度推荐搭配商品
     * 
     * @param currentProduct 当前商品描述
     * @param candidateProducts 候选商品列表
     * @return 推荐商品ID列表（Top 5）
     */
    @Async("virtualTaskExecutor")
    public CompletableFuture<java.util.List<String>> recommendRelatedProducts(
            String currentProduct,
            java.util.List<Map<String, String>> candidateProducts) {
        log.info("智能推荐关联商品 - 候选商品数: {}", candidateProducts.size());
        
        StringBuilder candidates = new StringBuilder();
        for (int i = 0; i < candidateProducts.size(); i++) {
            var product = candidateProducts.get(i);
            candidates.append(i + 1)
                     .append(". ID: ").append(product.get("id"))
                     .append(", 名称: ").append(product.get("name"))
                     .append(", 描述: ").append(product.get("description"))
                     .append("\n");
        }
        
        String prompt = String.format("""
            你是智能导购助手。请基于当前商品，从候选列表中推荐最相关的5个搭配商品。
            ...
            请返回推荐的商品ID列表，格式：["id1", "id2", "id3", "id4", "id5"]
            只输出JSON数组，不要有其他说明。
